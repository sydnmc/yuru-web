...
async function populateRow(i, mapStatus) { //i = row number
    ...
    if (imgSheetLink == "") {
        bgLink = "images/map-thumbnails/tamate.jpg" //replaces backgrounds with no link with えっとですねぇ～　たまてって名前は玉手箱が由来でして
    } else {
        bgLink = imgSheetLink;
    }
    ...
    if (plural) { //writing text for pluralized beatmaps
        ...
        for (let j = 0; j < plurality; j++) {
            additionalDiffText = additionalDiffText+`<a id="tab-diffname-${i}-${j}"></a><br>
        ...
        for (let j = 0; j < plurality; j++) {
            ...
            document.getElementById(`tab-diffname-${i}-${j}`).style = 'margin-bottom: 10px;';
