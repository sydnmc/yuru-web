<svelte:head>
  ...
  <meta property="og:description" content="my little place on the internet">
  <title>sydnmc△</title>
</svelte:head>

<script lang="ts">
    import { colourize } from "osu-colourizer";
    ...
    async function getRecentGd() {
        ...
        while (!foundRanked && i > 0) {
            if (gds[i].status === "ranked" || gds[i].status === "qualified") {
                //qualified should show up, since i basically treat it as being ranked :p
                latestRanked = gds[i];
                foundRanked = true;
            }
            i--;
...
<LegacyHeader person="syd" page="home"/>
    ...
    <div id="showcase">
      <div id="osu-showcase-wrapper">
        ...
        {#await getRecentGd()}
            ...
            <div class="gd-thumb" style="background-image: linear-gradient(90deg, rgba(0,0,0,0.5) 0%, rgba(0,0,0,0.5) 100%), url({gd?.bgLink}">
                ...
                <p class="gd-text" style="color: {colourize(gd?.maps[0].sr)}">{gd?.maps[0].diffname} | {gd?.maps[0].sr} ☆</p>
            </div>
            {:catch}
                <p>mreow</p>
            {/await}
      ...
      <div id="right-side">
        <div id="songs-preview" onclick={() => location.href = getPageRoot('syd')+'music'}>
...
<style>
    ...
    #right-side {
        /* this does come before in the html, but the width calculation is easier to understand this way~ */
        position: absolute;
        right: 0px;
        top: 0px;
        width: calc(
            40% - 24px - 18px
        ); /* gives a 18px gap between the left and right side, each have 10px padding and 4px border*/
    }
    ...
    .upper-ranked-text {
        ...
        width: calc(
            ((33% * 0.6) - 4px)
        ); /* close enough for now, but i wanna find a better solution with js, probably? */
        text-align: center;
    ...
    @media only screen and (max-device-width: 768px) {
        ...
        #osu-showcase-wrapper {
            width: calc(
                100% - 30px
            ); /* apparently this gets the spacing right, not sure why at all :sob: */
            margin-bottom: 15px;
        }
