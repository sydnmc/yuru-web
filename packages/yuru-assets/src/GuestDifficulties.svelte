    ...
    async function populateRow() {
        ...
        for (let i = 0; i < mapStatus.length; i++) {
            ...
            for (let j = 0; j < mapStatus[i].maps.length; j++) {
                ...
                if (diff.sr < 4.4 || typeof diff.sr === "string") {
                    //in cases where the map background text is too bright, or is wip (still too bright)
                    mapStatus[i].maps[j];
                }
            }
        }
...
<style>
    ...
    .ranked {
        background: linear-gradient(
            90deg,
            rgba(147, 196, 125, 0.8) 0%,
            rgba(147, 196, 125, 0) 100%
        );
    }
    .ranked p {
        color: var(--offwhite);
    ...
    .qualified {
        background: linear-gradient(
            90deg,
            rgba(109, 158, 235, 0.8) 0%,
            rgba(109, 158, 235, 0) 100%
        );
    }
